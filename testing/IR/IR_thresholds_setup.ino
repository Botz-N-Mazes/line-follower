...
const int maxReadingCount = 500; // Increased readings per sensor
...
void setup() {
  ...
  for (int sensorIndex = 0; sensorIndex < 6; sensorIndex++) {
    ...
    delay(2000); // Delay between calibrations for stability
  }
...
void loop() {
  // Main program logic can go here, using the calibrated thresholds
...
void calibrateSensor(int sensorIndex, float thresholds[]) {
  ...
  // Optional: Perform K-means clustering to determine centroids (if needed)
  float centroids[2] = {0, 0};
  calculateCentroids(sensorReadings, maxReadingCount, centroids);
  thresholds[sensorIndex] = (centroids[0] + centroids[1]) / 2;
