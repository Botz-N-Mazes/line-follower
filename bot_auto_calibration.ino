...
int baseSpeed = 100; // Base motor speed (adjust as needed)
...
void loop() {
  ...
  while (activeSensors == 0) {
    Serial.println("No sensors active, stopping motors");
    setMotorSpeeds(0, 0);
    delay(100);
    return;
  }
...
void calibrateSensor(int sensorIndex, float thresholds[]) {
  ...
  int count = 0; // Counter to track forward/backward cycles
  while (readingCounts < maxReadingCount) {
    // Forward motion for 10 readings
    if (count < 10) {
      Serial.println("Moving forward");
      setMotorSpeeds(80, 80); // Move forward
    }
    // Backward motion for the next 10 readings
    else if (count < 20) {
      Serial.println("Moving backward");
      setMotorSpeeds(-80, -80); // Move backward
    }
    // Read sensor value
    ...
    count++;
    if (count >= 20) {
      count = 0; // Reset after forward and backward cycle
    }
    delay(30); // Delay for stability
  }

// Stop motors after calibration is complete
setMotorSpeeds(0, 0);
